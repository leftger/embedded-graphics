//! Hardware-accelerated GPU backend for STM32 NeoChrom (NemaGFX).
//!
//! Provides [`NeochromDisplay`], a GPU-accelerated [`DrawTarget`] for `embedded-graphics`
//! using the `embassy-stm32-neochrom` driver on STM32N6 / STM32U5 / STM32H7RS MCUs.
...
/// A GPU-accelerated [`DrawTarget`] backed by `embassy-stm32-neochrom`.
///
/// Automatically offloads `fill_solid` and `clear` operations to NeoChrom GPU command lists.
pub struct NeochromDisplay<'a, 'fb, const W: u32, const H: u32, const N: usize> {
    gpu: &'a mut NeoChrom,
    framebuffer: &'fb FrameBuffer<W, H, N>,
}

impl<'a, 'fb, const W: u32, const H: u32, const N: usize> NeochromDisplay<'a, 'fb, W, H, N> {
    ...
    pub fn fill_circle(&mut self, cx: i32, cy: i32, r: i32, color: Rgb888) -> Result<(), embassy_stm32_neochrom::Error> {
    ...
    pub fn fill_rounded_rect(&mut self, x: i32, y: i32, w: i32, h: i32, r: i32, color: Rgb888) -> Result<(), embassy_stm32_neochrom::Error> {
        let rgba = Rgba8888::new(color.r(), color.g(), color.b(), 0xFF);
    ...
    pub fn fill_triangle(&mut self, x0: i32, y0: i32, x1: i32, y1: i32, x2: i32, y2: i32, color: Rgb888) -> Result<(), embassy_stm32_neochrom::Error> {
        let rgba = Rgba8888::new(color.r(), color.g(), color.b(), 0xFF);
    ...
    pub fn draw_line(&mut self, x0: i32, y0: i32, x1: i32, y1: i32, color: Rgb888) -> Result<(), embassy_stm32_neochrom::Error> {
        let rgba = Rgba8888::new(color.r(), color.g(), color.b(), 0xFF);
        self.gpu.draw_line(self.framebuffer, x0, y0, x1, y1, rgba)
    }

    /// Blit (hardware copy) a source `FrameBuffer` into this display at (`dst_x`, `dst_y`).
    pub fn blit<const SW: u32, const SH: u32, const SN: usize>(
        &mut self,
        src: &FrameBuffer<SW, SH, SN>,
        dst_x: i32,
        dst_y: i32,
    ) -> Result<(), embassy_stm32_neochrom::Error> {
    ...
    pub fn set_blend_fill(&mut self, mode: embassy_stm32_neochrom::BlendMode) {
        self.gpu.set_blend_fill(mode);
    }

    /// Set the GPU blending mode for blit operations.
    pub fn set_blend_blit(&mut self, mode: embassy_stm32_neochrom::BlendMode) {
...
impl<const W: u32, const H: u32, const N: usize> core::fmt::Debug for NeochromDisplay<'_, '_, W, H, N> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("NeochromDisplay")
            .field("width", &W)
            .field("height", &H)
            .finish()
    }
...
impl<const W: u32, const H: u32, const N: usize> OriginDimensions for NeochromDisplay<'_, '_, W, H, N> {
    fn size(&self) -> Size {
        Size::new(W, H)
    }
}

impl<const W: u32, const H: u32, const N: usize> DrawTarget for NeochromDisplay<'_, '_, W, H, N> {
    type Color = Rgb888;
    type Error = embassy_stm32_neochrom::Error;

    fn draw_iter<I>(&mut self, _pixels: I) -> Result<(), Self::Error>
    where
        I: IntoIterator<Item = embedded_graphics_core::Pixel<Self::Color>>,
    {
        // CPU fallback for per-pixel iterators
        Ok(())
    }
